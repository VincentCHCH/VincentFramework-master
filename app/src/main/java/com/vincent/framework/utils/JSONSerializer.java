...
public class JSONSerializer {
    ...
    public static JSONObject parseJSONData(String jsonData) {
        LogUtil.log(LogUtil.LogLevel.DEBUG,LOG_TAG, "parseJSONData by file name start");
        try {
            JSONObject json = new JSONObject(jsonData);
            //LogUtil.log(LogUtil.LogLevel.DEBUG,LOG_TAG, "parseJSONData: " + json);
            LogUtil.log(LogUtil.LogLevel.DEBUG,LOG_TAG, "parseJSONData successed");
            return json;
        } catch (JSONException e) {
            LogUtil.log(LogUtil.LogLevel.DEBUG,LOG_TAG, "parseJSONData.JSONException: " + e.getMessage());
            return null;
        }
    }
    ...
    public static <T> List<T> deserializeJSONArray(Class<T> elementClassType,
        ...
        for (int i = 0; i < array.length(); i++) {
            try {
                ...
                LogUtil.log(LogUtil.LogLevel.DEBUG,LOG_TAG, "deserializeJSONArray.JSONException: " + e.getMessage());
            	// keep it empty since we know it is the issue for not existed value
            	// continue the loop
            	
            } catch (IllegalAccessException e) {
                LogUtil.log(LogUtil.LogLevel.DEBUG,LOG_TAG, "deserializeJSONArray.IllegalAccessException: "
                        + e.getMessage());
                break;
            } catch (InstantiationException e) {
                LogUtil.log(LogUtil.LogLevel.DEBUG,LOG_TAG, "deserializeJSONArray.InstantiationException: "
                        + e.getMessage());
                break;
            }
        }
        ...
        try {
            ...
            if (actualClass == int.class || actualClass == Integer.class) {
                ...
                if (value != null)
                {
                    ...
                    {
                    	LogUtil.log(LogUtil.LogLevel.DEBUG,LOG_TAG, "name = " + name + "; value = " + value);
